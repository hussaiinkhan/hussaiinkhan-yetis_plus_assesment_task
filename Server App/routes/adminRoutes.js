...
router.post('/ads', asyncHandler(async (req, res) => {
  console.log(req);
  const { ad_title, area, description, is_rent } = req.body;
  ...
  try {
    ...
    const newAd = new Ad({ ad_title, area: coordinates, description, is_rent, image,user_id});
...
router.put('/ads/:id', asyncHandler(async (req, res) => {
  const { ad_title, area, description, is_rent, image } = req.body;
  try {
