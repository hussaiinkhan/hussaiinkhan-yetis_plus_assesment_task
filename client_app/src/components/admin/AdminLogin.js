...
const LoginPage = () => {
  ...
  const handleAdminLogin = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post('https://yetisplusapi.onrender.com/api/admin/login', {
        phone,
        password
      });
    ...
    <div className="flex min-h-screen bg-cover bg-center" style={{ backgroundImage: 'url(https://img.freepik.com/premium-vector/vector-hand-drawn-delicious-restaurant-food-background-pattern_690073-151.jpg?w=1380)' }}>
      <div className="hidden md:flex md:w-1/2 items-center justify-center bg-[#26ac81] bg-opacity-80 text-white p-8">
        <div>
          ...
          <p className="text-xl ml-16 font-md">Are you a Customer?<Link className='font-bold text-[#fad71a]' to='/'> Login here</Link></p>
        </div>
      </div>
      <div className="w-full md:w-1/2 bg-[#26ac81] bg-opacity-80 flex items-center justify-center p-8">
        <div className="bg-white p-8 rounded-lg shadow-lg w-full max-w-md">
          ...
          <form onSubmit={handleAdminLogin}>
            ...
            <p className='text-gray-700 font-medium text-md mb-2'>New here? <Link to='/adminsignup' className='font-bold text-[#26ac81]'>Signup</Link></p>
            <p className='sm:hidden text-gray-700 font-medium text-md mb-2'>Are you a Customer? <Link to='/' className='font-bold text-[#26ac81]'>Login here</Link></p>
